function exploratoryAnalysis(plotCase)
...
switch plotCase
    case 'population accuracy'
        ...
        bmap = plmColors(3, 'pastel1');
        hold on;
        X = 1:2;
        tmp = mean(acc,1); plotAcc(1,:) = tmp(1:3); plotAcc(2,:) = tmp(4:6);
        b = bar(X, plotAcc);
        sem = nanstd(acc, 1)/sqrt(nSubj) ; sem = reshape(sem, [2 3]);
        errorbar_pos = errorbarPosition(b, sem);
        errorbar(errorbar_pos', plotAcc, min(sem,1-plotAcc), sem, 'k','linestyle','none', 'lineWidth', 1.2);
        ylim([0 1]);
        legend('Baseline', 'Train', 'Test', 'Location', 'northeast');
    ...
    case 'population RT'
        ...
        bmap = plmColors(3, 'pastel1');
        hold on;
        tmp = mean(rt,1); plotRT(1,:) = tmp(1:3); plotRT(2,:) = tmp(4:6);
        b = bar([1 2], plotRT);
        sem = nanstd(rt, 1)/sqrt(nSubj); sem = reshape(sem, [2 3]);
        errorbar_pos = errorbarPosition(b, sem);
    ...
    case 'intrachunk RT'
        ...
        bmap = plmColors(3, 'pastel1');
        hold on;
        tmp = nanmean(rtChunkCorr,1); plotRTChunk(1,:) = tmp(1:3); plotRTChunk(2,:) = tmp(4:6);
        sem = nanstd(rtChunkCorr, 1) / sqrt(nSubj); sem = reshape(sem, [2 3]);
    ...
    case 'reward-complexity curve'
        ...
        hold on;
        scatter(complexity(:,1), reward(:,1), 120, 'filled', 'MarkerFaceColor', '#0072BD');
        polycoef = polyfit(complexity(:,1), reward(:,1), 2);
        X = linspace(1, 1.37, 50);
        Y = polycoef(1).*X.*X + polycoef(2).*X + polycoef(3);
        p1 = plot(X, Y, 'MarkerFaceColor', '#0072BD', 'LineWidth', 4);
      
        scatter(complexity(:,4), reward(:,4), 120, 'filled', 'MarkerFaceColor', '#D95319'); 
    ...
    case 'RT-complexity curve'
        ...
        hold on;
        scatter(complexity(:,4), time(:,4), 120, 'filled', 'MarkerFaceColor', '#0072BD');
        polycoef = polyfit(complexity(:,4), time(:,4), 2);
        X = linspace(1, 1.55, 100);
        Y = polycoef(1).*X.*X + polycoef(2).*X + polycoef(3);
        p1 = plot(X, Y, 'MarkerFaceColor', '#0072BD', 'LineWidth', 4);
      
        scatter(complexity(:,5), time(:,5), 120, 'filled', 'MarkerFaceColor', '#D95319'); 
        polycoef = polyfit(complexity(:,5), time(:,5), 2);
        X = linspace(1, 1.55, 100);
        Y = polycoef(1).*X.*X + polycoef(2).*X + polycoef(3);
        p2 = plot(X, Y, 'MarkerFaceColor', '#D95319', 'LineWidth', 4);
